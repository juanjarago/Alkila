...
export default function AvailabilityCalendar({ slug }: { slug: string }) {
  ...
  return (
    <section className="mt-4 rounded-[2rem] border border-[#C6C0B1] bg-[#F4EFE2] p-5 shadow-sm sm:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[#66752F]">
            Calendario
          </p>
          <h2 className="mt-1 text-lg font-black capitalize leading-tight text-[#17332A]">
            {monthTitle(month)}
          </h2>
        </div>
        ...
        <div className="flex shrink-0 gap-2">
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-full border border-[#C6C0B1] bg-white text-lg font-black text-[#17332A] transition hover:-translate-y-0.5"
      ...
      <div className="mt-4 grid grid-cols-7 gap-1 text-center text-[0.7rem] font-black uppercase text-[#66752F]">
        ...
        {cells.map((day, index) =>
          day ? (
            <div
              ...
              className={[
                "grid aspect-square min-h-9 place-items-center rounded-xl text-sm font-black",
                day.available
            ...
            <div key={`empty-${index}`} className="aspect-square min-h-9" />
          )
        )}
      </div>

      {loading ? (
        <div className="mt-3 rounded-2xl border border-[#C6C0B1] bg-white p-3 text-xs font-semibold text-[#4B544D]">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-[#4B544D]">
